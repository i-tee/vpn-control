...
`Dockerfile`, `docker-compose.yml`, `nginx/` в репо есть, но прод на них не
работает — это остатки от локальной разработки.
...
⚠️ `daysLeft = floor(balance / config('vpn.default_price'))` — считается по
**default_price**, а не по фактической сумме списания. У юзера с двумя каналами
оценка «дней осталось» будет завышена вдвое.
...
3. **Секреты в git:** `config/vpn.php` содержит боевой IP StrongSwan-сервера,
   порт и **захардкоженный дефолт `secret_key`** — при публичном репозитории это
   утечка. (`update_webhook.php` с токеном DEV-бота — в `.gitignore`, там ок.)
...
8. **`daysLeft` считается по default_price**, а не по фактическому списанию.
9. **Долг накапливается неограниченно** — списание идёт даже при глубоко
   отрицательном балансе.
10. **Нет тестов** — только стоковые ExampleTest.
11. **Мёртвый код:** `VpnTestController` + `views/vpn/test.blade.php` (роуты
...
12. **`telegram_username` unique** — юзер может сменить username в Telegram на
